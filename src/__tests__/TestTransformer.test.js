...
describe('TestTransformer', () => {
  ...
  it('should handle export default function correctly', () => {
      ...
      expect(result).toContain('function Component() { return <div>Hello</div>; }');
      // Verifica l'assegnazione a defaultExport
      expect(result).toMatch(/const defaultExport_src_component_jsx\s*=\s*(Component|function\s+Component)/);
  });
  ...
  it('should throw error if module is not found', () => {
    ...
    expect(() => transformer.transform('src/test.js')).toThrow(/Module not found/);
  });
  ...
  it('should generate aliases for default imports to fix scope issues', () => {
      const files = {
          ...
          'src/test.js': {
              path: 'src/test.js',
              isFolder: false,
              content: "import App from './App.jsx'; test('renders', () => { render(<App />); });"
          }
      };
  ...
  it('should wrap non-exporting files in block scope to prevent variable collisions', () => {
      ...
      const result = transformer.transformVirtual("import './src/testA.js'; import './src/testB.js';");
      ...
      expect(result).toMatch(/\{\s*const sharedVar = 'A';\s*\}/);
      expect(result).toMatch(/\{\s*const sharedVar = 'B';\s*\}/);
  });
});